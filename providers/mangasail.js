...
const HOMEPAGE_URL = 'https://www.sailmg.com/';
...
// Function to scrape latest chapters from homepage
// Function to scrape latest chapters from homepage
async function scrapeLatestChapters() {
  try {
    ...
    // Helper function to convert minicover URLs to full cover URLs
    const getFullCoverUrl = (thumbnailUrl) => {
      if (!thumbnailUrl) return null;
      
      // Convert styles/minicover to full cover path
      // From: /sites/default/files/styles/minicover/public/manga/cover/...
      // To: /sites/default/files/manga/cover/...
      const fullUrl = thumbnailUrl.replace(
        /\/sites\/default\/files\/styles\/minicover\/public\//,
        '/sites/default/files/'
      );
      
      return fullUrl.startsWith('http') ? fullUrl : `https://www.sailmg.com${fullUrl}`;
    };

    // Parse the nested list structure
    // Look for <li> elements containing manga info
    $('li').each((i, elem) => {
      const $li = $(elem);
      
      // Find the manga cover image link (parent manga)
      const $coverLink = $li.find('> a[href*="/content/"] img').parent();
      
      if ($coverLink.length > 0) {
        const $img = $coverLink.find('img');
        const thumbnail = $img.attr('src');
        const thumbnailUrl = getFullCoverUrl(thumbnail);
        
        // Get manga title from the nested structure
        const $mangaTitle = $li.find('.tl a strong').first();
        const mangaName = $mangaTitle.text().trim();
        
        // Find all chapter links in the #c-list
        const $chapterList = $li.find('ul#c-list > li');
        
        $chapterList.each((j, chapterElem) => {
          const $chapter = $(chapterElem);
          const $chapterLink = $chapter.find('a[href*="/content/"]').first();
          ...
          if (href) {
            ...
            // Get the time posted
            const dateText = $chapter.find('.tm').text().trim();
            
            // Extract chapter number from title
            const chapterNum = title.match(/chapter\s*(\d+\.?\d*)/i)?.[1] || 
                              title.match(/ch\.?\s*(\d+\.?\d*)/i)?.[1] ||
                              title.match(/\s(\d+\.?\d*)$/)?.[1];
            ...
            if (title && chapterId) {
              latestChapters.push({
                ...
                chapterId: chapterId,
                url: fullUrl,
                mangaName: mangaName || extractMangaName(title),
    ...
    // Fallback: If no chapters found with the above method, try alternative selectors
    if (latestChapters.length === 0) {
      console.log('Trying alternative extraction methods...');
      
      // Method 2: Look for any chapter links with time stamps
      $('a[href*="/content/"]').each((i, elem) => {
        const $link = $(elem);
        const href = $link.attr('href');
        const text = $link.text().trim();
        
        if (href && text && href.includes('/content/') && !href.includes('?page=')) {
          const fullUrl = href.startsWith('http') ? href : `https://www.sailmg.com${href}`;
          const chapterId = href.split('/content/')[1]?.split('?')[0] || '';
          
          // Look for nearby time element
          const $parent = $link.closest('li');
          const dateText = $parent.find('.tm, time, .date').text().trim();
          
          // Look for nearby thumbnail
          const $listParent = $link.closest('ul').closest('li');
          const thumbnail = $listParent.find('img').first().attr('src');
          const thumbnailUrl = getFullCoverUrl(thumbnail);
          
          // Get manga name from strong tag in same parent
          const mangaName = $listParent.find('.tl strong').text().trim();
          
          // Filter out navigation and manga title links
          if (chapterId && text.length > 3 && /\d+/.test(text)) {
            const chapterNum = text.match(/chapter\s*(\d+\.?\d*)/i)?.[1] || 
                              text.match(/ch\.?\s*(\d+\.?\d*)/i)?.[1] ||
                              text.match(/\s(\d+\.?\d*)$/)?.[1];
            ...
            latestChapters.push({
              ...
              chapterId: chapterId,
              url: fullUrl,
              mangaName: mangaName || extractMangaName(text),
              chapterNumber: chapterNum || null,
              thumbnail: thumbnailUrl,
              datePosted: dateText || null,
              href: href
    ...
    // Remove duplicates based on chapterId
    const uniqueChapters = Array.from(
      new Map(latestChapters.map(item => [item.chapterId, item])).values()
    );

    return uniqueChapters;
  } catch (error) {
    throw new Error(`Failed to scrape latest chapters: ${error.message}`);
  }
}

// Helper function to extract manga name from chapter title
function extractMangaName(title) {
  // Remove chapter numbers and common patterns
  let mangaName = title
...
router.get('/', (req, res) => {
  res.json({
    provider: 'SailMG',
    version: '1.0.0',
    features: ['anilist-integration', 'pagination', 'batch-download', 'latest-chapters'],
    endpoints: {
      ...
      'GET /latest-chapters': '🆕 Get latest chapters from homepage',
    ...
    examples: {
      ...
      latestChapters: '/manga/sailmg/latest-chapters'
    }
  });
});

// Get latest chapters from homepage
router.get('/latest-chapters', async (req, res) => {
  try {
    console.log('Fetching latest chapters from homepage...');
    const latestChapters = await scrapeLatestChapters();
    ...
    if (latestChapters.length === 0) {
      return res.json({
        ...
        count: 0,
        chapters: [],
        message: 'No chapters found on homepage. The site structure may have changed.'
      });
    }
