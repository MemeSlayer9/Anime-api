...
router.get('/latest-releases', async (req, res) => {
  try {
    ...
    const url = `https://ww2.mangafreak.me/Latest_Releases/${page}`;
    ...
    $('.latest_releases_item').each((i, elem) => {
      const $item = $(elem);
      
      // Extract manga title and link
      const $titleLink = $item.find('.latest_releases_info > a');
      const mangaTitle = $titleLink.find('strong').text().trim();
      const mangaUrl = $titleLink.attr('href');
      
      // Extract series name from manga URL
      const seriesMatch = mangaUrl?.match(/\/Manga\/([^\/]+)/);
      const seriesName = seriesMatch ? seriesMatch[1] : '';
      
      // Build full cover image URL using series name and proxy it
      const originalImageUrl = seriesName 
        ? `https://images.mangafreak.me/manga_images/${seriesName.toLowerCase()}.jpg`
        : '';
      const fullImageUrl = originalImageUrl 
        ? `${baseUrl}/manga/mangafreak/proxy?url=${encodeURIComponent(originalImageUrl)}`
        : '';
      
      // Extract chapter info
      const $chapterLink = $item.find('.latest_releases_info a[href*="Read1_"]');
      const chapterText = $chapterLink.text().trim();
      const chapterUrl = $chapterLink.attr('href');
      
      // Extract chapter number and series from chapter URL
      const chapterMatch = chapterUrl?.match(/Read1_([^_]+(?:_[^_]+)*)_(\d+(?:\.\d+)?)/);
      const chapterSeries = chapterMatch ? chapterMatch[1] : '';
      const chapterNumber = chapterMatch ? chapterMatch[2] : '';
      
      // Extract release time
      const releaseTime = $item.find('.latest_releases_time').text().trim();
      
      // Build full URLs
      const fullMangaUrl = mangaUrl?.startsWith('http') ? mangaUrl : `https://ww2.mangafreak.me${mangaUrl}`;
      const fullChapterUrl = chapterUrl?.startsWith('http') ? chapterUrl : `https://ww2.mangafreak.me${chapterUrl}`;
      ...
      releases.push({
        ...
        chapter: {
          chapterId: chapterSeries && chapterNumber ? `${chapterSeries}/${chapterNumber}` : '',
    ...
    console.error('MangaFreak latest releases error:', error.message);
    res.status(500).json({
      success: false,
